  ...
  for(c in c("None", "DCV")){
    ...
    if (length(unique(ab$animal))>=1){
      ...
      if (!file.exists(here("models", sprintf("%s_%s_ViralLoad_nonphylo_presence.Rdata", g, c)))) {
        ...
        save(model_presence, file = here("models", sprintf("%s_%s_ViralLoad_nonphylo_presence.Rdata", g, c)))
      ...
      if (!file.exists(here("models", sprintf("%s_%s_ViralLoad_nonphylo_abundance.Rdata", g, c)))) {
        ...
        save(model_abundance, file = here("models", sprintf("%s_%s_ViralLoad_nonphylo_abundance.Rdata", g, c)))
        
      } else {load(here("models", sprintf("%s_%s_ViralLoad_nonphylo_abundance.Rdata", g, c)))}
      
      
      # Presence marginal
      var_pr_marginal_viral <- model_presence$VCV[,1]
      var_pr_marginal_pr <- model_presence$VCV[,4]
      cov_pr_marginal_pr <- model_presence$VCV[,2]
      ...
      # Abundance marginal
      var_ab_marginal_viral <- model_abundance$VCV[,1]
      var_ab_marginal_ab <- model_abundance$VCV[,4]
      cov_ab_marginal_ab <- model_abundance$VCV[,2]
